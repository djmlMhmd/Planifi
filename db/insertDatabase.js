...
const {executeCustomQuery, executeCustomQueryWithData} = require("./utils/database.utils");
...
const insertDatas = () => {
    ...
    for(let pro of pros){
        try{
            ...
            const valuesUser = [
                pro.firstName, pro.lastName, pro.password,
                pro.email, pro.phone, pro.country, pro.city, pro.address
            ]
            executeCustomQueryWithData(insertUserQuery, valuesUser)
                .then(result => {
                    ...
                    executeCustomQueryWithData(insertProQuery, valuesPro).then();
    ...
    for(let client of clients){
        const values = [
            client.firstName, client.lastName, client.password,
            client.email, client.phone, client.country, client.city, client.address
        ]
    ...
    for(let service of services){
        const values = [
            ...
            service.duration, service.professional_id
        ]
        executeCustomQueryWithData(insertService, values).then()
    }
    console.log("services inserés")

    const insertReservation = 'INSERT INTO reservations (professional_id, start_time, end_time, users_id, service_id, day_of_week) VALUES ($1, $2, $3, $4, $5, $6)'


    for(let reservation of reservations){

        const values = [
            reservation.professional_id, reservation.start_time, reservation.end_time,
            reservation.users_id, reservation.service_id, reservation.day_of_week
        ]
