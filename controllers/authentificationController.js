...
const {createToken, REGISTRATION_EXPIRES_IN, JWT_COOKIE_EXPIRES_IN, verifyJWT, decodeJWT} = require("../utils/auth.utils");
...
module.exports.register_client_post = async (req, res) => {
    ...
    try{
        ...
        if (resultExistUser.rows.length === 1) {
            errorLogger(`Un compte existe déjà avec cette adresse mail ${body.email}:`, '', 'authentification.js', '/inscription/utilisateur', constants.POST_HTTP)
            return sendFailure(res, `Vous possédez déjà un compte à cette adresse mail`)
        }
    }
    catch (e) {
        errorLogger(`Erreur lors de la vérification à l'inscription : ${e.stack}`, '', 'authentification.js', '/inscription/utilisateur', constants.POST_HTTP)
    ...
    try {
        ...
        if(isUndefinedOrEmpty(body.company_name)  || isUndefinedOrEmpty(body.company_address) ){
            ...
            try{
                ...
                resultInsertUser = await client.query(insertUserQuery, valuesUser);
                const insertProQuery = "INSERT INTO pro_account(company_name, company_address, user_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING RETURNING *"
                const valuesPro = [
                    body.company_name,
                ...
                errorLogger(`Erreur lors de l'inscription en tant que professionnel : ${e.stack}`, '', 'authentification.js', '/inscription/utilisateur', constants.POST_HTTP)
                return sendInternalServerError(res, "Erreur serveur lors de l'inscription. " + e.message)
            }
        }
        ...
        if (resultInsertUser && resultInsertUser.rowCount > 0) {
            logLogger(`Utilisateur inscrit avec succès:` + JSON.stringify(resultInsertUser.rows[0]) , '', 'authentification.js', '/inscription/utilisateur', constants.POST_HTTP)
             ...
             * @type {string}
             */
            const token = createToken(id, '', constants.CONFIRM_REGISTRATION, REGISTRATION_EXPIRES_IN)
...
module.exports.register_pro_post = async (req, res) => {
    ...
    if (error) {
        errorLogger(`Erreur lors de la validation des données de l'utilisateur ${id}:`, '', 'authentification.js', '/inscription/professionnel', constants.POST_HTTP)
        return sendBadRequest(res, error.details[0].message)
    }
    const client = getClientsCollection();
    ...
    try{
        ...
        if (resultExistUser.rows.length === 1) {
            errorLogger('Vous possédez déjà un compte professionnel à cette adresse mail', '', 'authentification.js', '/inscription/professionnel', constants.POST_HTTP)
            return sendFailure(res, `Vous possédez déjà un compte professionnel à cette adresse mail`)
        }
    }
    catch (e) {
        errorLogger(`Erreur lors de la vérification à l'inscription : ${e.stack}`, '', 'authentification.js', '/inscription/professionnel', constants.POST_HTTP)
        return sendInternalServerError(res, "Erreur serveur lors de l'inscription. " + e.message)
    }
    ...
    try {
        ...
        if (resultExistUser.rows.length === 1) {
            ...
            if (result.rowCount > 0) {
                ...
                sendSuccess(res, "Votre inscription a bien été prise en compte.")
            } else {
                ...
                return sendInternalServerError(res, 'Le compte existe déjà ou une autre erreur est survenue.')
            }
        }
        else {
            errorLogger(`Aucun compte existe avec cet ID ${id}:`, '', 'authentification.js', '/inscription/professionnel', constants.POST_HTTP)
...
module.exports.connexion_post = async (req, res) => {
    ...
    try {
        ...
        if (clientQueryResult.rows.length === 1) {
            ...
            if (!est_verifie) {
                warnLogger(`L'utilisateur ${users_id} essaye de se connecter en étant pas vérifié`, '', 'authentification.js', '/connexion', constants.POST_HTTP)
            ...
            if (match) {
                ...
                logLogger(`Authentification réussie (${email})`, '', 'authentification.js', '/connexion', constants.POST_HTTP)
                return sendSuccessWithNoContent(res)
            }
            else{
...
module.exports.forgot_password_post = async (req, res) => {
    ...
    try {
        ...
        if(userQueryresult.rowCount > 0) {
             ...
             * @type {string}
             */
            const token = createToken(users_id, '', constants.FORGOT_PASSWORD, REGISTRATION_EXPIRES_IN)

            sendSuccess(res, `Un lien de réinitialisation de votre mot de passe a bien été envoyé votre adresse mail: ${email}`)
            await sendResetPassword(email, firstName, `${process.env.API_URL}/forgot-password-reset?token=${token}`)
        }
        else {
    ...
    if(type === constants.FORGOT_PASSWORD) {
        ...
        try {
            ...
            if(updatePasswordResult.rowCount === 0){
                errorLogger(`Echec lors de la mise à jour du mot de passe de l'utilisateur ${id} `, '','authentification.js', '/forgot-password-reset', constants.PUT_HTTP)
                return sendFailure(res, 'Echec de la mise à jour du mot de passe')
            }
            return sendSuccessWithNoContent(res)
        }
        catch (e) {
            errorLogger(`Echec lors de la mise à jour du mot de passe de l'utilisateur ${id}: ${e}`, '','authentification.js', '/forgot-password-reset', constants.PUT_HTTP)
            return sendInternalServerError(res, `Echec lors de la mise à jour du mot de passe de l'utilisateur ${id}`)
        }
    }
